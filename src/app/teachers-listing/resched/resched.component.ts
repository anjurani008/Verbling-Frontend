...
import { BookRescheduleModalComponent } from 'src/app/dashboard/book-reschedule-modal/book-reschedule-modal.component';
const colors: any = { 
  red: {
    primary: '#ad2121',
    secondary: '#FAE3E3',
...
@Component({
  selector: 'app-resched',
  templateUrl: './resched.component.html',
  styleUrls: ['./resched.component.scss'],
  providers: [
    DayService,
    WeekService,
    WorkWeekService,
...
export class ReschedComponent implements OnInit {
  ...
  constructor(
    ...
    @Optional() public dialogRef: MatDialogRef<ReschedComponent>,
  ...
  getSlot() {
    ...
    this.teacherService.getSlot(`schedules?teacherId=${this.Teacherdata.teacherId}&notAssigned=true&isDeleted=false`).subscribe(res => {
  ...
  openDialog(data) {
    console.log(data);
      ...
      const dialogRef = this.dialog.open(BookRescheduleModalComponent, {
 
      width: '25%',
      data: {
        data: data,
        key:this.Teacherdata.key,
        appointmentId: this.Teacherdata.appointmentId
      },
    });
